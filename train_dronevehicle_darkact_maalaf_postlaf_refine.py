# 训练（DroneVehicle DarkAct V2 LAF 融合后零初始化残差精炼）
# 对应模型 YAML：yaml/yolov8s-DarkAct-MAA2D-LAFMerge-Refine-P345-R4-v1.yaml
import inspect
import os
...
# P3/P4/P5：LAF fused -> 1x1(C->C/4) -> 3x3(C/4->C) -> gamma residual -> FPN/PAN。
# gamma=0 使新增 Refine 从恒等映射开始，且不改变 LAF 对 RGB/IR backbone 的反馈。
CHECKPOINT = str(repo_path("pre-pth/yolov8s-obb_twostream_darkact_staticmaa_laffeedback_refine_p345_e025_v1.pt"))
EXPERIMENT_NAME = (
    "DarkAct_StaticMAA2DLAFMergeFeedbackRefine_P345_H2-4-8_"
    "RefineE0p25-K1-3-G0-MSK3-5-R4-PosBeta-DW3D1-2-2_v1"
)
...
# project/name 遵循命名.txt；普通训练参数保留在 args.yaml，不写入实验名。
results = monitor.run(
    tracked_train,
    model,
    "PT-R012",
