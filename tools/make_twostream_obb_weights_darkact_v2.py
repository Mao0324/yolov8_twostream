#!/usr/bin/env python3
"""Migrate YOLOv8s-OBB weights to the DarkAct StaticMAA/LAF-feedback V2 model.

The new StaticMAA2D and LAFMergeFeedback2D modules keep their initialized
parameters. Compatible single-stream backbone weights are copied into the RGB
and IR branches, and compatible neck/OBB tensors are copied into shared layers.
"""
...
DEFAULT_TARGET_YAML = ROOT / "yaml/yolov8s-DarkAct-MAA2D-LAFMerge-P345-R4-v2.yaml"
DEFAULT_OUTPUT = ROOT / "pre-pth/yolov8s-obb_twostream_darkact_staticmaa_laffeedback_p345_h2-4-8_r4_v2.pt"
...
def _copy_mapped_weights(
    source: Mapping[str, torch.Tensor], target: StateDict, layer_map: Mapping[int, int], tag: str
    ...
    print(f"[{tag}] copied_by_layer=" + ", ".join(f"{src}->{dst}:{n}" for (src, dst), n in per_layer.items()))
...
def main() -> int:
    ...
    # C2f -> C2f_Faster intentionally has non-overlapping internal key names;
    # compatible tensors are copied while unmatched IR-only tensors retain
    # their target initialization. Shared RGB/neck mappings must be complete.
    if rgb_missing:
