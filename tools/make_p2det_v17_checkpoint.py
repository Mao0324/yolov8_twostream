#!/usr/bin/env python3
"""Build the embedded-architecture P2Det V17 checkpoint from DarkACT LAF."""
...
from ultralytics.models.yolo.obb.p2det_object_reliability_train import P2ObjectReliabilityOBBModel
...
TARGET_YAML = ROOT / "yaml/yolov8s-P2Det-ObjectReliabilityPrompt-P34-MonotonicPriorLAF-NoGDER-v17.yaml"
OUTPUT = ROOT / "pre-pth/yolov8s-obb_twostream_p2det_objectreliability_p34_monotonicpriorlaf_v17.pt"
...
def validate_architecture(model):
    ...
    if len(model.model) != 36:
        raise RuntimeError(f"V17 target has {len(model.model)} layers; expected 36")
    errors = []
    ...
    for index in (10, 15):
        ...
        if module.rgb_prompt_head.body[-1].bias is not None or module.ir_prompt_head.body[-1].bias is not None:
            errors.append(f"layer {index}: V17 Prompt output must be bias-free")
        if float(module.merge.prompt_strength.detach()) <= 0:
            errors.append(f"layer {index}: monotonic Prompt strength is not positive")
    if errors:
...
def main():
    ...
    for label, path in (("DarkACT source", SOURCE), ("V17 YAML", TARGET_YAML)):
        if not path.is_file():
            raise FileNotFoundError(f"{label} not found: {path}")

    torch.manual_seed(17)
    source = YOLO(str(SOURCE), task="obb").model.float().eval()
    target = P2ObjectReliabilityOBBModel(str(TARGET_YAML), ch=3, nc=None, verbose=True).float().eval()
    ...
    with torch.no_grad():
        for key, value in source_state.items():
            if key not in target_state or target_state[key].shape != value.shape:
                raise RuntimeError(f"DarkACT tensor cannot map into V17: {key} {tuple(value.shape)}")
    ...
    # Zero Prompt heads emit P=[0.5,0.5]. Equal log-priors cancel in the
    # modality softmax, so the whole detector exactly preserves DarkACT.
    ...
    if forward_difference != 0.0:
        raise RuntimeError(f"V17 neutral forward differs from DarkACT: max_abs_diff={forward_difference}")
    ...
    migration = {
        ...
        "darkact_v17_max_abs_diff": maximum,
        "neutral_forward_max_abs_diff": forward_difference,
        "target_scale": "s",
        "object_reliability_version": 17,
    }
    ...
    print(f"darkact_v17_max_abs_diff={maximum}")
