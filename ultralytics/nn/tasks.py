...
#            print("ch[f]", f, ch[f[0]])
            ...
            args = [c2, *args]
        elif m is CFGPCrossAttentionFusion:
            if not isinstance(f, list) or len(f) != 2:
                raise ValueError("CFGPCrossAttentionFusion requires exactly two source layers")
            c2 = ch[f[0]]
            if ch[f[1]] != c2:
                raise ValueError(f"CFGPCrossAttentionFusion source channels differ: {c2} vs {ch[f[1]]}")
            configured_c2 = make_divisible(min(args[0], max_channels) * width, 8)
            if configured_c2 != c2:
                raise ValueError(
                    f"CFGPCrossAttentionFusion configured channels {configured_c2} do not match inputs {c2}"
                )
            args = [c2, *args[1:]]
        ...
        if twostream_branch_active:
            ...
            elif m in {ADD, LAFMerge2D, FTCrossMerge, CFGPCrossAttentionFusion}:
